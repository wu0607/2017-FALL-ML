...
def calc_lowest_entropy(data, feature_index):	
	...
	for i in range(0,len(data)):
		...
		curr_entropy = calc_entropy(left)*float(len(left))/float(len(data)) + calc_entropy(right)*float(len(right))/float(len(data))
...
def ID3(root):
	...
	while curr_node != None:
		...
		curr_node = find_impure_leaf(root)
	#print 'ID3 finish'		
#make random forset	by ID3 Algorithm
def Random_forest(root):
	#print 'Random_forest start'
	Forest = list()
	for _ in range (15):
		training_set = root[:40]
...
def predict_and_vote(datapoint, Forest):
	curr_node = Forest
	label = [0, 0, 0]
	#print root
	...
	for tree in Forest:
		...
		if curr_node.label == 'Iris-setosa':
			label[0] += 1
		elif curr_node.label == 'Iris-versicolor':
			label[1] += 1
		elif curr_node.label == 'Iris-virginica':
			label[2] += 1
	
	if label[0]>label[1] and label[0]>label[2]:
		return 'Iris-setosa'
	elif label[1]>label[0] and label[1]>label[2]:
		return 'Iris-versicolor'
	elif label[2]>label[0] and label[2]>label[1]:
		return 'Iris-virginica'
	
def calc_error(data, Forest):
	...
	for datapoint in data:
		prediction = predict_and_vote(datapoint, Forest)
...
def main():
	...
	for _ in range(test_time):
		for i in range(k_fold_num):	
			Forest = Random_forest(training_set[i])
			
			accuracy, precision, recall = calc_error(testing_set[i], Forest)
			acc_final += accuracy
			for j in range(3):
